...
void afficherMenu() {
    ...
    std::cout << "  " << CYAN   << "[1]" << RESET << " BFS       — tous les chemins courts, fourmis en parallele\n";
...
int choisirFourmiliere(const std::vector<std::string>& fichiers) {
    ...
    std::cout << BOLD << CYAN;
    std::cout << "  ╔══════════════════════════════════════════╗\n";
    std::cout << "  ║         Choisir une fourmiliere          ║\n";
    std::cout << "  ╚══════════════════════════════════════════╝\n";
    std::cout << RESET << "\n";
...
void afficherCarte(
    ...
    const std::string& vestibule, const std::string& dortoir,
    ...
    for (auto& salle : salles) {
        ...
        if (!fourmisIci.empty()) {
            ...
            for (auto& f : fourmisIci) {
                if (shown >= 6) { std::cout << "+" << (fourmisIci.size()-6) << " autres"; break; }
                std::cout << "🐜" << f << " "; shown++;
            }
            std::cout << RESET;
        }
...
void simulerAvecVisuel(Anthill& anthill, int algo) {
    ...
    for (int i = 0; i < (int)steps.size(); i++) {
        ...
        std::cout << DIM << "  [ Entree = etape suivante | q+Entree = quitter ]" << RESET << " ";
    ...
    std::cout << "  " << BOLD << nbAnts << RESET << " fourmi(s) -> dortoir en "
              << BOLD << YELLOW << steps.size() << " etape(s)" << RESET << "\n\n";
    std::cout << DIM << "  [ Entree pour continuer... ]" << RESET;
